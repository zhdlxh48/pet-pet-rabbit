...
namespace FMODUnity
{
    ...
    public class Settings : ScriptableObject
    {
        ...
        // Checks whether the native FMOD libraries for the current Unity platform exist.
        public bool VerifyPlatformLibsExist()
        {
            string libraryPath;

#if UNITY_EDITOR
            Platform platform = CurrentEditorPlatform;
#else
            Platform platform = FindCurrentPlatform();
#endif

            if (!PlatformLibsExist(platform, out libraryPath))
            {
                Debug.LogWarning("[FMOD] Unable to locate '" + libraryPath + "'.");
        ...
        private static bool PlatformLibsExist(Platform platform, out string libraryPath)
        {
            libraryPath = platform.GetLibraryPath();

            return string.IsNullOrEmpty(libraryPath) || File.Exists(libraryPath);
        }
        ...
        private bool CanBuildTarget(BuildTarget target, out string error)
        {
            ...
            if (!PlatformLibsExist(platform, out libraryPath))
            {
                error = string.Format("No FMOD library for build target {0} (couldn't find {1}).\n" +
                            "You may need to reinstall the relevant integration package from {2}.",
                            target, libraryPath, DownloadURL);
                return false;
            }
        ...
        public class BuildPreprocessor : IPreprocessBuildWithReport
        {
            ...
            {
                string error;
                if (!Settings.Instance.CanBuildTarget(report.summary.platform, out error))
                {
        ...
        public class BuildPreprocessor : IPreprocessBuild
        {
            ...
            {
                string error;
                if (!Settings.Instance.CanBuildTarget(target, out error))
                {
                    throw new BuildFailedException(error);
                }
            }
        ...
        public class BuildTargetChecker : IActiveBuildTargetChanged
        {
            ...
            {
                string error;
                if (!Settings.Instance.CanBuildTarget(current, out error))
                {
